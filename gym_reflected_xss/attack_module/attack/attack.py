...
class JSparser:
    ...
    def rec_dict_search(self,dic):
        ...
        if type(dic) == dict:
                        ...
                        if dic['arguments']:
                            for elem in dic['arguments']:
                                if elem['raw'] == '0727':
                                    ...
                                    return True
                    """
                    if dic['callee']['name'] == "eval":
                        if dic['arguments']:
                            for elem in dic['arguments']:
                                if elem['raw'] in "alert(0727);confirm(0727);prompt(0727);"
                                    found = True
                                    return True
                    """
                if not found:
                    keys=dic.keys()
                    for key in keys:
                        if self.rec_dict_search(dic[key]):
                            found = True
                        if found: break
            except: 
                keys=dic.keys()
            ...
            for elem in dic:
                keys=elem.keys()
                for key in keys:
                    if self.rec_dict_search(elem[key]):
                        found = True
                    if found: break
                if found: break
    ...
    def jsparse(self,jscode):
        try:
            parsed = parse(jscode)
            
            if self.rec_dict_search(parsed):
                return True
            else: return False
        except:
            return False
...
class Attack:
    ...
    # BASE_DIR = os.path.dirname(sys.modules["attack_module"].__file__)
...
class Mutator:
    ...
    def mutate(self, request: Request, refererUsed = False):
    ...
    def refererMutate(self, request: Request, refererUsed = False):
        ...
        if isinstance(self._payloads, tuple):
            ...
            original_flags = self._payloads[1]
        elif isinstance(self._payloads, list):
            payload = self._payloads[0][0]
            original_flags = self._payloads[0][1]
        ...
        return evil_req, "Referer Header", payload, flags

    def urlMutate(self, request: Request):
        ...
        # url redirection 시 "/" 를 뒤에 안붙여주면 301 redirection response
        evil_req = Request(
        ...
        return evil_req, "URL", payload, flags  

    def allMutate(self, request: Request):
        ...
        if request.method == "GET":
            ...
            params_list = post_params

        for i in range(len(params_list)):
            # heuristic for constant value
            if params_list[i][1] == None or len(params_list[i][1]) < 15:
                params_list[i][1] = "injection" + str(i) + "here"
        
        refererPayload = "injection" + str(len(params_list)) + "here"
        ...
        evil_req = Request(
            ...
            post_params=post_params,
            file_params=file_params,
            referer=refererPayload,
            link_depth=request.link_depth
        )

        return evil_req, params_list  

    def allMutateWithChar(self, request: Request):
